    ...
    fetch(`/api/playlists`)
      ...
      .then((data) => {
        ...
        setPlaylist({
          ...
          tracks: (found.tracks || []).map(dbTrackToTrack),
        });
      })
      .catch(() => {})
      ...
      <div className="from-muted/50 to-background border-b bg-gradient-to-b p-6 lg:p-8">
        <div className="flex flex-col gap-6 md:flex-row md:items-end">
          ...
          <div className="flex-1">
            ...
            <h1 className="mb-4 text-4xl font-bold text-balance lg:text-5xl">{playlist.title}</h1>
        ...
        <div className="mt-6 flex items-center gap-4">
          <Button size="lg" className="rounded-full" onClick={() => playPlaylist(playlist)}>
      ...
      <ScrollArea className="h-[calc(100%-320px)]">
        <div className="p-6 lg:p-8">
          <div className="text-muted-foreground mb-4 grid grid-cols-[auto_1fr_auto_auto] gap-4 border-b pb-2 text-sm">
            ...
            <div>Title</div>
            <div>Album</div>
            <div className="flex items-center gap-2"><Clock className="h-4 w-4" /></div>
          </div>
          <div className="space-y-1">
            {playlist.tracks.map((track, index) => (
              ...
              >
                <div className="text-muted-foreground flex w-8 items-center justify-center text-sm">{index + 1}</div>
                <div className="flex min-w-0 items-center gap-3">
                  <img src={track.coverUrl || "/placeholder.svg"} alt={track.title} className="h-10 w-10 rounded object-cover" />
                  <div className="min-w-0">
                    <p className="truncate font-medium">{track.title}</p>
                    <p className="text-muted-foreground truncate text-sm">{track.artist}</p>
                  </div>
                </div>
                <div className="text-muted-foreground flex items-center text-sm">{track.album}</div>
                <div className="text-muted-foreground flex items-center text-sm">{formatTime(track.duration)}</div>
              </button>
            ))}
          </div>
        </div>
