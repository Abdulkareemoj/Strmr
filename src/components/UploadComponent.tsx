...
export default function UploadComponent({ uploadType, maxFileSize }: UploadComponentProps) {
  ...
  const onUpload = async () => {
    ...
    for (const file of files) {
      try {
        ...
        toast.success(`${uploadType === 'videos' ? 'Video' : 'Short'} ${file.name} uploaded successfully`)
      } catch (error) {
  ...
  const onDelete = async (file: UploadedFile) => {
    try {
      ...
      toast.success(`${uploadType === 'videos' ? 'Video' : 'Short'} ${file.name} deleted successfully`)
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      {previews.length > 0 && (
        <div className="space-y-4">
          ...
          <Button onClick={onUpload} disabled={uploading}>
            {uploading ? "Uploading..." : `Upload Selected ${uploadType === 'videos' ? 'Videos' : 'Shorts'}`}
          </Button>
        </div>
      )}
      ...
      <UploadedFilesCard uploadedFiles={uploadedFiles} onDelete={onDelete} uploadType={uploadType} />
    </div>
  )
}
    ...
    <div className="rounded-lg border bg-card text-card-foreground shadow-sm">
      <div className="flex flex-col space-y-1.5 p-6">
        ...
        <p className="text-sm text-muted-foreground">
          View and manage your uploaded {uploadType === 'videos' ? 'videos' : 'shorts'} here
        </p>
      </div>
      <div className="p-6">
        {uploadedFiles.length > 0 ? (
                ...
                >
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{uploadType === 'videos' ? 'Video' : 'Short'}</p>
                    </div>
                  </div>
          ...
          <div className="py-10 text-center">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Get started by uploading a {uploadType === 'videos' ? 'video' : 'short'}.
            </p>
          </div>
        )}
      </div>
